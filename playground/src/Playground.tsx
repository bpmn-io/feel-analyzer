...
export function Playground({
  ...
  const [expressionType, setExpressionType] = useState<'expression' | 'unaryTest'>(() => {
    if (initialExpressionType) return initialExpressionType;
    return (localStorage.getItem(STORAGE_KEY_EXPRESSION_TYPE) as 'expression' | 'unaryTest') || 'expression';
  });
  ...
  useEffect(() => {
    localStorage.setItem(STORAGE_KEY_ANALYSIS_COLLAPSED, String(analysisCollapsed));
  }, [analysisCollapsed]);

  useEffect(() => {
    localStorage.setItem(STORAGE_KEY_SYNTAX_TREE_COLLAPSED, String(syntaxTreeCollapsed));
  }, [syntaxTreeCollapsed]);

  useEffect(() => {
    localStorage.setItem(STORAGE_KEY_BACKEND_COLLAPSED, String(backendCollapsed));
  }, [backendCollapsed]);
  ...
  useEffect(() => {
    localStorage.setItem(STORAGE_KEY_AUTO_UPDATE_CONTEXT, String(autoUpdateContext));
  }, [autoUpdateContext]);
  ...
  // Auto-update context by adding missing keys with null values (if enabled)
  useEffect(() => {
    if (!autoUpdateContext) {
      return;
    }

    if (!analysisData || !analysisData.neededInputs || analysisData.neededInputs.length === 0) {
      return;
    }
    ...
    try {
      ...
      const neededRootVars = new Set(analysisData.neededInputs.map((input: string) => input.split('.')[0]));

      // Start with all existing context (preserve everything)
      const newContext: any = { ...contextObj };

      // Only add missing needed keys with null values
      neededRootVars.forEach((rootVar) => {
        // If key already exists, don't touch it
        if (rootVar in newContext) {
          return;
        }

        // Check if any input has nested properties
        const hasNestedProps = analysisData.neededInputs.some((input: string) => input.startsWith(rootVar + '.'));

        if (hasNestedProps) {
          // Create nested object structure with null values
          ...
          nestedProps.forEach((prop: string) => {
            newContext[rootVar][prop] = null;
          });
        } else {
          // Add with null value - user can remove by leaving null
          newContext[rootVar] = null;
        }
      });

      // Filter out null values for non-needed variables, keep nulls for needed variables
      const cleanContext: any = {};
      Object.keys(newContext).forEach((key) => {
        const value = newContext[key];

        // If this key is not needed and is null, skip it
        if (!neededRootVars.has(key) && value === null) {
          return;
        }

        if (typeof value === 'object' && value !== null) {
          // For objects, keep all nested properties including nulls if needed
      ...
      const newContextString = JSON.stringify(cleanContext, null, 2);
      const oldContextString = JSON.stringify(contextObj, null, 2);
      if (newContextString !== oldContextString) {
        setContext(newContextString);
      }
    } catch (error) {
      console.error('Error auto-updating context:', error);
    }
  }, [analysisData, autoUpdateContext]);
  ...
  const handlePrefillContext = () => {
    ...
    try {
      ...
      const missingInputs = analysisData.neededInputs.filter((input: string) => {
      ...
      inputsByRoot.forEach((inputs, rootVar) => {
        ...
        // Check if any input has nested properties
        const hasNestedProps = inputs.some((input: string) => input.includes('.'));

        if (hasNestedProps) {
          // Create nested object structure
          contextString += `  "${rootVar}": {\n`;
          const properties = new Set(
            inputs.filter((input: string) => input.includes('.')).map((input: string) => input.split('.').slice(1).join('.')),
          );
          properties.forEach((prop) => {
            contextString += `    "${prop}": null,\n`;
          });
          contextString += '  },\n';
        } else {
          // Simple value based on type - use null as placeholder
          contextString += `  "${rootVar}": null,\n`;
        }
      });
  ...
  const handleEvaluate = async () => {
    ...
    try {
      ...
      const configuredParser = parserDialect === 'standard' ? parser : parser.configure({ dialect: parserDialect });
    ...
    if (!backendUrl) {
      setEvaluationResult({
        error: "Backend not configured. Click '⚙️ Configure' to set up the FEEL Scala Playground backend.",
      });
    } else {
      try {
        const contextObj = JSON5.parse(context);
      ...
      <div class="layout">
        <div class="input-panel">
          <div class="panel-header">
            <h2>Input</h2>
            <button onClick={handleShare} class="btn-share" title="Copy shareable link to clipboard">
          ...
          <div class="form-group">
            <label>
              Expression Type:
              <select value={expressionType} onChange={(e) => setExpressionType((e.target as HTMLSelectElement).value as any)}>
          ...
          <div class="form-group">
            <label>
              Parser Dialect:
              <select value={parserDialect} onChange={(e) => setParserDialect((e.target as HTMLSelectElement).value as any)}>
          ...
          <div class="form-group">
            <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
              <input type="checkbox" checked={useBuiltins} onChange={(e) => setUseBuiltins((e.target as HTMLInputElement).checked)} />
              <span>Use Camunda Built-ins</span>
            </label>
            {useBuiltins && (
              <div style="font-size: 0.85em; color: #666; margin-top: 4px; padding-left: 24px;">
                Functions like uuid(), trim(), etc. will be filtered from needed inputs
              </div>
            )}
          </div>
          ...
          <div class="form-group">
            <label>
              Expression:
              <textarea value={expression} onInput={(e) => setExpression((e.target as HTMLTextAreaElement).value)} rows={3} />
            </label>
          </div>
            ...
            <div class="form-group">
              <label>
                ...
                <input
                  ...
                  onInput={(e) => setUnaryTestValue((e.target as HTMLInputElement).value)}
                  placeholder="e.g., 5"
                />
              </label>
            </div>
          ...
          <div class="form-group">
              ...
              <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 4px;">
                ...
                <label style="display: flex; align-items: center; gap: 4px; font-size: 0.9em; font-weight: normal; cursor: pointer;">
                  <input
                    ...
                    onChange={(e) => setAutoUpdateContext((e.target as HTMLInputElement).checked)}
                  />
                  Auto-update
                </label>
              </div>
            ...
            <div style="display: flex; gap: 8px; margin-top: 8px;">
              <button onClick={handleClearContext} class="btn-secondary" style="flex: 1;" title="Clear context">
                🗑️ Clear
              </button>
              {analysisData && analysisData.neededInputs && analysisData.neededInputs.length > 0 && (
                <button
        ...
        <div class="output-panel">
          ...
          <div class="output-section">
            <h3 class="collapsible-header" onClick={() => setAnalysisCollapsed(!analysisCollapsed)}>
              <span class="collapse-icon">{analysisCollapsed ? '▶' : '▼'}</span>
              Analysis Result{' '}
              {performanceMetrics.analysisTime !== undefined && (
                <span class="perf-badge">{performanceMetrics.analysisTime.toFixed(2)}ms</span>
              )}
            </h3>
            {!analysisCollapsed && <pre>{analysisResult || '// Waiting for input...'}</pre>}
          </div>

          <div class="output-section">
            <h3 class="collapsible-header" onClick={() => setSyntaxTreeCollapsed(!syntaxTreeCollapsed)}>
              <span class="collapse-icon">{syntaxTreeCollapsed ? '▶' : '▼'}</span>
              Syntax Tree
            </h3>
            {!syntaxTreeCollapsed && <pre>{syntaxTree || '// Waiting for input...'}</pre>}
          </div>

          <div class="output-section">
            <h3 class="collapsible-header" onClick={() => setBackendCollapsed(!backendCollapsed)}>
              ...
              {performanceMetrics.backendTime !== undefined && (
                <span class="perf-badge">{performanceMetrics.backendTime.toFixed(2)}ms</span>
              )}
            </h3>
            {!backendCollapsed && (
              <>
          ...
          <div class="output-section">
            <h3 class="collapsible-header" onClick={() => setFeelinCollapsed(!feelinCollapsed)}>
              <span class="collapse-icon">{feelinCollapsed ? '▶' : '▼'}</span>
              Evaluation Result (feelin){' '}
              {performanceMetrics.feelinTime !== undefined && <span class="perf-badge">{performanceMetrics.feelinTime.toFixed(2)}ms</span>}
            </h3>
            {!feelinCollapsed && (
              <>
